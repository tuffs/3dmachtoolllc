...
export async function getSurtaxPercent(zipCode) {
  ...
  if (!zipCode) {
    return { success: false, error: 'You must provide a zip code to proceed.', surtax: 0.00, finalMultiplier: 0.00 };
  }
  ...
  if (isNaN(zipCodeInt) || !validator.isPostalCode(zipCode.toString(), 'US')) {
    return { success: false, error: 'You have entered an invalid U.S. postal code.', surtax: 0.0, finalMultiplier: 0.00 };
  }
  ...
  try {
    ...
    if (!taxRecord) {
      ...
      return { success: false, error: 'No surtax found for this zip code', surtax: 0.00, finalMultiplier: 0.00 };
    }

    const surtax = taxRecord.rate;
    const totalRate = baseRate + surtax;
    return { success: true, surtax, finalMultiplier: 1 + totalRate }; // e.g. 1 + (0.06 + 0.015) = 1.075
  } catch (error) {
    console.error('Error fetching surtax rate:', error);
    return { success: false, error: 'Failed to fetch surtax rate', surtax: 0.0, finalMultiplier: 0.00 };
  }
}